...
# Global backup for RGB values
last_rgb_values = [130, 40, 170, 255, 0]
...
def save_log(note=""):
    ...
    log_entry = f"{timestamp} - RGB: ({r:03d}, {g:03d}, {b:03d}, Alpha: {a:03d}, Vignette: {v:03d}) - Note: {note or user_text}\n"
...
def set_color_single(channel):
    global last_rgb_values, rgb_mode_active
    if rgb_mode_active:
        last_rgb_values = [red_slider.get(), green_slider.get(), blue_slider.get(), alpha_slider.get(), vignette_slider.get()]
        rgb_mode_active = False
    if channel == 'red':
        red_slider.set(last_rgb_values[0])
        green_slider.set(0)
        blue_slider.set(0)
        save_log("Set RED only")
    elif channel == 'green':
        red_slider.set(0)
        green_slider.set(last_rgb_values[1])
        blue_slider.set(0)
        save_log("Set GREEN only")
    elif channel == 'blue':
        red_slider.set(0)
        green_slider.set(0)
        blue_slider.set(last_rgb_values[2])
        save_log("Set BLUE only")
    update_color()
...
def restore_rgb():
    ...
    green_slider.set(last_rgb_values[1])
    blue_slider.set(last_rgb_values[2])
    alpha_slider.set(last_rgb_values[3])
    vignette_slider.set(last_rgb_values[4])
    rgb_mode_active = True
    save_log("Restored RGB")
    update_color()

def key_handler(event):
    key = event.char.lower()
    if key == 'r':
        set_color_single('red')
    elif key == 'g':
        set_color_single('green')
    elif key == 'b':
        set_color_single('blue')
    elif key == 'a':
        restore_rgb()
...
root.bind_all("<Control-r>", lambda event: set_color_single('red'))
root.bind_all("<Control-g>", lambda event: set_color_single('green'))
root.bind_all("<Control-b>", lambda event: set_color_single('blue'))
#root.bind_all("<Control-a>", lambda event: restore_rgb)
root.bind_all("<Control-Shift-A>", lambda event: restore_rgb())
...
log_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "color_log.txt")
timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
log_entry = f"{timestamp} ==================Session Start==================\n"
with open(log_file_path, "a") as log_file:
        log_file.write(log_entry)
...
# Add control buttons for color channels before exit button
channel_frame = tk.Frame(root)
channel_frame.pack(pady=10)
red_only_btn = tk.Button(channel_frame, text="Red", underline=0, width=10, command=lambda: set_color_single('red'))
red_only_btn.grid(row=0, column=0, padx=5)
green_only_btn = tk.Button(channel_frame, text="Green", underline=0, width=10, command=lambda: set_color_single('green'))
green_only_btn.grid(row=0, column=1, padx=5)
blue_only_btn = tk.Button(channel_frame, text="Blue", underline=0, width=10, command=lambda: set_color_single('blue'))
blue_only_btn.grid(row=0, column=2, padx=5)
rgb_restore_btn = tk.Button(channel_frame, text="A RGB", underline=0, width=10, command=restore_rgb)
...
def toggle_cycle():
    ...
    if cycle_running:
        rgb_restore_btn.config(state="disabled")
        red_only_btn.config(state="disabled")
        green_only_btn.config(state="disabled")
        blue_only_btn.config(state="disabled")
        cycle_rgb()
        cycle_button.config(text="Stop Cycle")
    else:
        rgb_restore_btn.config(state="normal")
        red_only_btn.config(state="normal")
        green_only_btn.config(state="normal")
        blue_only_btn.config(state="normal")
        cycle_button.config(text="Start Cycle")
        save_log("RGB Cycle stopped")
...
cycle_running = False
cycle_index = 0
cycle_button = tk.Button(root, text="Start Cycle", width=20, command=toggle_cycle)
